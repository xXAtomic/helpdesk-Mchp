...
<div class="max-w-4xl mx-auto py-12 px-6">
    ...
    <div class="mb-12">
        ...
        <p class="text-slate-500 font-medium tracking-wide">Registro de equipamiento en el inventario global.</p>
    </div>

    <!-- Formulario Pro-SaaS -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all hover:shadow-md">
        <form action="{{ route('admin.inventory.store') }}" method="POST" class="p-8 space-y-8">
            @csrf
            ...
            <div class="space-y-2">
                <label class="text-[0.7rem] font-bold text-slate-400 uppercase tracking-widest ml-1">Asignar a Usuario (Opcional)</label>
                <select name="user_id"
                    class="w-full px-5 py-4 bg-slate-50 border-none rounded-xl text-slate-900 font-semibold focus:ring-2 focus:ring-indigo-500 transition-all">
