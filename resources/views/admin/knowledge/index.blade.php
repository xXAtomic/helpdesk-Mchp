...
<div class="px-6 py-8 bg-slate-950 min-h-screen text-slate-200">
    ...
    <div class="flex flex-col lg:flex-row gap-10">
        ...
        <div class="lg:w-2/3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach($manuals as $article)
                    @php $path1 = $article->file_path ? asset('storage/' . $article->file_path) : ''; @endphp
                    <div onclick="openKnowledgeModal('{{ addslashes($article->title) }}', '{{ addslashes($article->content) }}', '{{ $article->category }}', '{{ $article->icon }}', '{{ $path1 }}', '{{ addslashes($article->file_name) }}')"
        ...
        <div class="lg:w-1/3">
            ...
            <div class="space-y-6">
                @foreach($tips as $tip)
                    @php $path2 = $tip->file_path ? asset('storage/' . $tip->file_path) : ''; @endphp
                    <div onclick="openKnowledgeModal('{{ addslashes($tip->title) }}', '{{ addslashes($tip->content) }}', '{{ $tip->category }}', '{{ $tip->icon }}', '{{ $path2 }}', '{{ addslashes($tip->file_name) }}')"
